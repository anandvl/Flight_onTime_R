...
library(plyr)
library(stringr)
#setwd('/home/rstudio') # set working dir to current directory.  Notice the forward slashes.
...
#  User input
# **********************************************
dataDir <- 'shiny/Flight_onTime/data'
yyyymmOfInt <- '201801'
...
# **********************************************
#  Collect all data for specific airport of interest and only include
#  data for those that airlines whose monthly flights were more than
...
plt <- ggplot(rearrData1, aes(x=Airl, y=DURATION, fill=TYPE)) +
  ...
  xlab('Airlines') +
  ylab('Duration (mins)') +
  ggtitle('Stats for Denver - Denver International for 2018-01')

print(plt)
png(paste('DIA_plot','png',sep='.'), width=1600, height=900)
print(plt)
dev.off()
